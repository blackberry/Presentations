...
static uint8_t* burst_bufs[1000] = { 0 };
static uint64_t burst_sizes[1000] = { 0 };
...
handle_photo_done_event(bps_event_t *event) {
    ...
    touch = false;
    state = STATE_VIEWFINDER;
    // save all of the jpegs that we cached during burst capture...
    int i;
    for (i=0; i<burst_frame_count; i++) {
        int fd;
...
run_state_machine()
{
    ...
    case STATE_VIEWFINDER:
        ...
        // if the user touches the screen anywhere, take a set of burst captures.
        if (touch) {
            touch = false;
            // zero our burst frame counter
            burst_frame_count = 0;
            err = camera_take_burst(handle,
                                    5,        // NUMBER OF BURST FRAMES TO CAPTURE
                                    &shutter_callback,
                                    NULL,
                                    NULL,
                                    &still_callback,
