...
void BarcodeDecoderControl::onCameraOpened() {
	...
	_landscapePreviewFrames= false;
	if (_camera->devicePreviewFrameDirection() == DisplayDirection::East || _camera->devicePreviewFrameDirection() == DisplayDirection::West) {
		_landscapePreviewFrames = true;

	}
	qDebug() << "+++++++ Previewframes in landscape: " << _landscapePreviewFrames << endl;
