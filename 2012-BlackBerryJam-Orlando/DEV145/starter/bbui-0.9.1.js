...
//
// Adam Stanley (Apr 30): Bug with Ripple extension
//		It is not using the correct appVersion - showing up as Chrome instead of BB10
//		Adding check for screen resolution to target BB10 Dev Alpha.
//
			bb.device.isBB10 = ((navigator.appVersion.indexOf('Version/10.0') >= 0) || (window.innerWidth == 768 && window.innerHeight == 1280));
		}
    ...
    doLoad: function(element) {
		...
		//Adam Stanley (Apr 29, 2012):
		//	Fix for issue #74 - https://github.com/blackberry/bbUI.js/issues/74
		bb.textInput.apply(root.querySelectorAll('input[type=tel],input[type=url],input[type=email],input[type=number],input[type=date],input[type=time],input[type=datetime],input[type=month],input[type=datetime-local],input[type=color]'));
...
bb.actionBar = {
	...
	apply: function(actionBar, screen) {
		...
		// Create the back button if it has one
		if (actionBar.hasAttribute('data-bb-back-caption')) {
		...
		if (limit > 5) {
			...
			/*if (res == 'lowres') {
				action.setAttribute('data-bb-img','images/actionBar/overflow_'+ color +'_theme.png');
			} else {
				action.setAttribute('data-bb-img','images/actionBar/overflow_'+ color +'_theme_'+res+'.png');
			}*/
