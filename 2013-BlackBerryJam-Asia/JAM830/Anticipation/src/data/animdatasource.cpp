...
void AnimDataSource::addInfo(const QString title, const QString band, int rating) {
	...
	if (!title.isEmpty() && !title.isNull()) {
		...
		QString query = "INSERT INTO animinfo (title, band, rating, revision_id, newItem) VALUES (:title, :band, :rating, :revision_id, :newItem)";
...
void AnimDataSource::markAsOld(int id) {
	...
	// Mark the table entry at id as old. New item is only used the
	// first time the item is shown to be able to trigger an animation on it.
	QVariantList dataList;
