...
int OpenGLView::initGL()
{
    ...
    if (m_egl_ctx == EGL_NO_CONTEXT) {
		OpenGLThread::eglPrintError("eglCreateContext");
        return EXIT_FAILURE;
    }
	...
	for (int index = 0; index < numberDisplays; index++) {
    	...
    	qDebug()  << "OpenGLView::initGL: display: " << ":" << index << ":" << displayID << ":" << m_display;
    	...
    	if (returnCode) {
			...
			if (displayID == m_display) {
			    ...
			    if (type == SCREEN_DISPLAY_TYPE_HDMI) {
			    	...
			    	qDebug()  << "OpenGLView::initGL: m_screen_win (HDMI): " << ":" << m_screen_win << ":" << m_screen_ctx << ":" << returnCode;
			    	...
			    	qDebug()  << "OpenGLView::initGL: m_screen_win: " << ":" << m_screen_win << ":" << returnCode;
	...
	qDebug()  << "OpenGLView::initGL: "<< m_screen_ctx << ":" << m_egl_disp << ":" << m_egl_conf << ":" << m_egl_ctx << ":" << m_screen_win;
...
void OpenGLView::setDisplay(VIEW_DISPLAY display)
{
	...
	qDebug()  << "OpenGLView::setDisplay: "<< m_display;
}
...
void OpenGLView::onVisible()
{
	qDebug()  << "OpenGLView::onVisible";
}
...
{
	//qDebug()  << "OpenGLView::renderView: " << m_renderCount << initialized() << " : " << enabled() << " : " << visible() << " : " << stale();
