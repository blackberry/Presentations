...
int GoodCitizen::initialize() {
	...
	qDebug()  << "GoodCitizen::initialize " << ":" << returnCode;
...
void GoodCitizen::enable_2D_left() {
    glViewport(0, 0, m_width, m_height / 2);

    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();

    glOrthof(0.0f, (float)m_width / (float)m_height / 2, 0.0f, 1.0f, -1.0f, 1.0f);

    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();
    glScalef(1.0f / (float)m_height / 2.0, 1.0f / (float)m_height / 2.0, 1.0f);
	glRotatef(5.0 * M_PI / 180.0, 0.0f, 1.0f, 0.0f);
}

void GoodCitizen::enable_2D_right() {
    glViewport(0, m_height / 2, m_width, m_height / 2);
...
void GoodCitizen::enable_3D_left() {
    glViewport(0, 0, m_width, m_height / 2.0);

    GLfloat aspect_ratio = (16.0 / 9.0); // multiply by two to deal with stereo halving distortion
...
void GoodCitizen::enable_3D_right() {
    glViewport(0, m_height / 2.0, m_width, m_height / 2.0);
...
void GoodCitizen::handleScreenEvent(bps_event_t *event) {
	...
	switch (screenEvent) {
		case SCREEN_EVENT_MTOUCH_TOUCH:
			...
			fprintf(stderr, "touch: %d %d,%d  %d,%d %f\n", touchID, realPosition[0], realPosition[1], position[0], position[1] );

			break;

		case SCREEN_EVENT_MTOUCH_MOVE:

			fprintf(stderr, "touch move: %d %d,%d  %d,%d %f\n", touchID, realPosition[0], realPosition[1], position[0], position[1] );
			break;

		case SCREEN_EVENT_MTOUCH_RELEASE:

			fprintf(stderr, "touch release: %d %d,%d  %d,%d %f\n", touchID, realPosition[0], realPosition[1], position[0], position[1] );
			break;
