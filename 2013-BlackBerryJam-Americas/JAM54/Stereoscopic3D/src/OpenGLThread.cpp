...
void OpenGLThread::update() {
	...
	if (viewCount > 0) {
		...
		qDebug()  << "OpenGLThread::update: attached: " << attached;

		for(index = 0; index < m_views.size(); index++) {
			qDebug()  << "OpenGLThread::update: check attached: " << attached << ":" << index << ":" << m_views.at(index)->display();
			...
			qDebug()  << "OpenGLThread::update: check attached: " << attached << ":" << index << ":" << m_views.at(index)->display() << ":" << m_views.at(index)->visible();
		}
		...
		for (int index = 0; index < m_views.size(); index++) {
			if (!m_views.at(index)->initialized() && m_views.at(index)->visible()) {
				...
				qDebug()  << "OpenGLThread::update: initialize: " << index << ":" << m_views.at(index)->display() << ":" << m_views.at(index)->visible() << ":" << returnCode;
				if (returnCode == EXIT_FAILURE) {
					qDebug()  << "OpenGLThread::update: view initialization failed: " << index;
					m_views.at(index)->setVisible(false);
					m_views.at(index)->setStale(false);
					setStopped(true);
				}
			}
			...
			if (m_views.at(index)->altered() && m_views.at(index)->visible()) {
				...
				qDebug()  << "OpenGLThread::update: regenerate: " << index << ":" << m_views.at(index)->display() << ":" << m_views.at(index)->altered() << ":" << m_views.at(index)->visible();
			}
