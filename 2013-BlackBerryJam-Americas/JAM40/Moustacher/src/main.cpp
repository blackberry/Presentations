...
static int moustache_width;
static int moustache_height;
static unsigned char *moustache;
                              ...
                              void *arg)
{
    ...
    if (format == CAMERA_METAFORMAT_FACE) {
        ...
        if (camera_meta_get_face_region(metadata, &region) != EOK) {
            ...
            state->found = true;
            // get roll if available
            state->roll = 0;
            camera_meta_get_face_roll(metadata, &state->roll);
...
// this filter makes a guess at where to put a moustache on the frame.
// Since face-detect can be a bit jittery, the moustache may appear and disappear from frame to frame
...
static void apply_filter(camera_buffer_t* inbuf,
                         camera_buffer_t* outbuf)
{
    ...
    if (state.found) {
        // given that we know the bounds of the face, let's estinate where
        // we figure a moustache would go.
        // first, determine center of face
        float fcx = state.fx + state.fw / 2;
        float fcy = state.fy + state.fh / 2;
        // now estimate width & height of moustache
        float mw = state.fw / 1.5;
        float mh = state.fh / 8;
        // and then estimate moustache center...
        // the moustache offset from center is going to be ~50% of the way to the edge
        float mdelta = state.fh / 4;
        float mcx = fcx - mdelta*sin(state.roll / 180 * M_PI);
        float mcy = fcy + mdelta*cos(state.roll / 180 * M_PI);
        state.debounce = 10;
        state.fx = mcx - mw/2;
        state.fy = mcy - mh/2;
        state.fw = mw;
    ...
    if (state.debounce > 0) {
        state.debounce--;
        // draw the moustache
        // this is just a very basic texture scaling algorithm in fixed-point
        uint32_t du = (moustache_width << 16) / state.fw;
        uint32_t dv = (moustache_height << 16) / state.fh;
        // note, PNG textures load upside-down.. so flip the V vector around
        uint32_t v = (moustache_height-1) << 16;
        // figure out a rough slope based on the reported roll value
        float dyofs = 0;
        if (fabs(state.roll)<=45) {
            dyofs = tan(state.roll / 180 * M_PI);
        }
        // since pivot is around center of moustache, need to offset the starting y co-ordinate by a
        // fixed amount each time around the loop.
        // note that we're not REALLY doing a proper rotation here.. it is actually a vertical skew, but
        // it should be sufficient to demonstrate the intended purpose since most faces are expected
        // to be close to vertical.
        float syofs = -(state.fw/2)*dyofs;
        for (int y = state.fy; y < state.fy + state.fh; y++, v-=dv) {
            ...
            for (int x = state.fx; x < state.fx + state.fw; x++, u+=du) {
                ...
                // clip to screen
                int screen_y = y + yofs;
                if ((screen_y < 0) || (screen_y >= (int)outbuf->framedesc.nv12.height)) continue;
                // determine texture pixel @ u,v
                int offset = moustache_width * 4 * (v>>16);
                offset += 4 * (u>>16);
                // skip if alpha is transparent
                if (moustache[offset+3] != 0) {
                    // note, we're not REALLY rendering black here.  we're just setting the Y channel
                    // to 0.  There still are valid UV components, but the resulting colour on-screen
                    // is close enough to black for my liking.
                    // a proper approach here would be to convert the RGB value to YUV, and then update
                    // the Y plane and UV plane appropriately.
                    // an even better approach would be to convert the source YUV pixel to RGB,
                    // do an alpha-blend with the moustache RGB, and then convert back to YUV/NV12.
                    // since my moustache png is basically a solid black + white image, there is no
                    // need to get that fancy here.
                    outbuf->framebuf[outbuf->framedesc.nv12.stride * screen_y + x] = 0;
                }
            }
        }
    }
...
init_camera()
{
    ...
    for (i=0; i<num; i++) {
        ...
        if (res[i].width > res[i].height) {
            if ((fabs(res[i].height - 720) < fabs(res[best].height - 720)) &&
                (fabs(res[i].width - 1280) < fabs(res[best].width - 1280))) {
                best = i;
            }
        } else {
            if ((fabs(res[i].width - 720) < fabs(res[best].width - 720)) &&
                (fabs(res[i].height - 1280) < fabs(res[best].height - 1280))) {
                best = i;
            }
        }
    }
...
main(int argc, char **argv)
{
    ...
    // load up the moustache image
    if (bbutil_load_png("app/native/moustache.png", &moustache_width, &moustache_height, &moustache) != EXIT_SUCCESS)
    {
        fprintf(stderr, "failed to load png\n");
        return EXIT_FAILURE;
    }
    fprintf(stderr, "loaded %d x %d moustache\n", moustache_width, moustache_height);
