...
SET deviceip=%1
SET password=%2
SET tmpfolder=C:\Temp\sample_application
...
ECHO.
ECHO Package name: %tmpfolder%\device\sample_application.bar
ECHO.
...
ECHO.
ECHO ...
"c:\program files\Research In Motion\BlackBerry 10 WebWorks SDK 1.0.4.7\dependencies\tools\bin\blackberry-deploy" -installApp -launchApp -device %deviceip% -password %devicepwd% -package %tmpfolder%\device\sample_application.bar
