...
string TemplateJS::InvokeMethod(const string& command) {
	...
	if (strCommand == "startLED") {

		return m_pTemplateController->startLED(arg);

	} else if (strCommand == "stopLED") {

		return m_pTemplateController->stopLED(arg);

	} else if (strCommand == "blinkCount") {
		// if arg exists we are setting property
		if (arg != strCommand) {
			m_pTemplateController->setBlinkCountProperty(arg);
		} else {
			return m_pTemplateController->getBlinkCountProperty();
		}

	/** END LED CODE **/


	} else if (strCommand == "testString") {
